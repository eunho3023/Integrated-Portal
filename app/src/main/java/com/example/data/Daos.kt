...
interface AppDao {
    ...
    // Timetables (학급 시간표)
    @Query("SELECT * FROM timetables WHERE schoolId = :schoolId AND grade = :grade AND classNum = :classNum")
    ...
    // Friends (친구 관리)
    @Query("SELECT * FROM friends WHERE myUid = :myUid")
    fun getFriendsFlow(myUid: String): Flow<List<FriendEntity>>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertFriend(friend: FriendEntity)

    @Query("DELETE FROM friends WHERE myUid = :myUid AND friendUid = :friendUid")
    suspend fun deleteFriend(myUid: String, friendUid: String)
}
