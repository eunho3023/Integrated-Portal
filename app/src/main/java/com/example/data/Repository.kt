...
class AppRepository(private val appDao: AppDao) {
    ...
    private fun mapSnapshotToUser(snapshot: DataSnapshot): UserEntity {
        ...
        val grade = snapshot.child("grade").value?.toString() ?: "1"
        val classNum = snapshot.child("classNum").value?.toString() ?: "1"
        val num = snapshot.child("num").value?.toString() ?: "1"
        return UserEntity(uid, username, displayName, role, schoolId, createdAt, phoneNumber, password, grade, classNum, num)
    }
    ...
    // Timetables
    fun getTimetableFlow(schoolId: String, grade: String, classNum: String): Flow<List<TimetableEntity>> =
        appDao.getTimetableFlow(schoolId, grade, classNum)

    suspend fun insertTimetable(timetable: TimetableEntity) {
        appDao.insertTimetable(timetable)
        try {
            ref?.child("timetables")?.child(timetable.schoolId)?.child(timetable.id)?.setValue(timetable)
        } catch (e: Exception) {
            Log.e("RealtimeDBSync", "Error inserting timetable", e)
        }
    }

    suspend fun insertTimetables(timetables: List<TimetableEntity>) {
        appDao.insertTimetables(timetables)
        try {
            timetables.forEach { item ->
                ref?.child("timetables")?.child(item.schoolId)?.child(item.id)?.setValue(item)
            }
        } catch (e: Exception) {
            Log.e("RealtimeDBSync", "Error inserting timetables", e)
        }
    }

    suspend fun deleteTimetable(timetable: TimetableEntity) {
        appDao.deleteTimetable(timetable)
        try {
            ref?.child("timetables")?.child(timetable.schoolId)?.child(timetable.id)?.removeValue()
        } catch (e: Exception) {
            Log.e("RealtimeDBSync", "Error deleting timetable", e)
        }
    }

    // Friends
    fun getFriendsFlow(myUid: String): Flow<List<FriendEntity>> = appDao.getFriendsFlow(myUid)

    suspend fun addFriend(myUid: String, friendUid: String, friendName: String, friendPhone: String = "") {
        val id = "$myUid-$friendUid"
        val friend = FriendEntity(id, myUid, friendUid, friendName, friendPhone)
        appDao.insertFriend(friend)
        try {
            ref?.child("friends")?.child(myUid)?.child(friendUid)?.setValue(friend)
        } catch (e: Exception) {
            Log.e("RealtimeDBSync", "Error adding friend", e)
        }
    }

    suspend fun removeFriend(myUid: String, friendUid: String) {
        appDao.deleteFriend(myUid, friendUid)
        try {
            ref?.child("friends")?.child(myUid)?.child(friendUid)?.removeValue()
        } catch (e: Exception) {
            Log.e("RealtimeDBSync", "Error removing friend", e)
        }
    }
}
