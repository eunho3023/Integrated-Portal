    ...
    private fun startGlobalUsersSync() {
        ...
        val listener = object : com.google.firebase.database.ChildEventListener {
            ...
            override fun onChildRemoved(snapshot: com.google.firebase.database.DataSnapshot) {
                ...
                scope.launch {
                    val existing = appDao.getUserById(uid)
                    if (existing != null) {
                        appDao.deleteUser(existing)
                    }
                }
            }
            ...
            override fun onCancelled(error: com.google.firebase.database.DatabaseError) {
                Log.w("RealtimeSync", "Global users sync cancelled", error.toException())
            }
        }
        usersRef.addChildEventListener(listener)
    }

    private fun syncUserFromSnapshot(snapshot: com.google.firebase.database.DataSnapshot) {
        val uid = snapshot.key ?: return
        ...
        if (uid.isNotEmpty() && (username.isNotEmpty() || displayName.isNotEmpty())) {
            val user = UserEntity(uid, username, displayName, role, schoolId, createdAt, phoneNumber, password, grade, classNum, num)
            scope.launch {
