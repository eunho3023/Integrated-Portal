...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    var callState by mutableStateOf("IDLE") // "IDLE", "RINGING_INCOMING", "RINGING_OUTGOING", "CONNECTED"
    var callingPeerName by mutableStateOf("")
    var isSpeakerphoneOn by mutableStateOf(true)

    // Meal Date State
    var selectedMealDateYmd by mutableStateOf(SimpleDateFormat("yyyyMMdd", Locale.getDefault()).format(Date()))
    var selectedMealDateFormatted by mutableStateOf(SimpleDateFormat("yyyy년 MM월 dd일 (E)", Locale.KOREAN).format(Date()))

    // Friends Flow
    val friendsList: StateFlow<List<FriendEntity>> = snapshotFlow { currentUser?.uid ?: "" }
        .flatMapLatest { uid ->
            if (uid.isNotEmpty()) repository.getFriendsFlow(uid) else flowOf(emptyList())
    ...
    fun signup(
        ...
        viewModelScope.launch {
            ...
            // Check if displayName (name/nickname) taken
            val cleanDisplayName = displayName.trim()
            if (allUsers.value.any { it.displayName.equals(cleanDisplayName, ignoreCase = true) }) {
                onError("이미 등록된 이름/닉네임입니다. 다른 이름이나 닉네임을 사용해 주세요.")
                return@launch
            }

            // Check if phoneNumber taken
            val cleanPhoneDigits = phoneNumber.filter { it.isDigit() }
            if (cleanPhoneDigits.isNotEmpty() && allUsers.value.any { it.phoneNumber.filter { ch -> ch.isDigit() } == cleanPhoneDigits }) {
                onError("이미 등록된 전화번호입니다. 다른 전화번호를 사용해 주세요.")
                return@launch
            }
            ...
            // Create user with Grade, Class, Number
            ...
            val newUser = UserEntity(
                ...
                password = pin,
                grade = if (grade.isNotBlank()) grade else "1",
                classNum = if (classNum.isNotBlank()) classNum else "1",
    ...
    fun updateCurrentUserInfo(
        ...
        viewModelScope.launch {
            ...
            val updatedUser = user.copy(
                ...
                grade = if (newGrade.isNotBlank()) newGrade.trim() else user.grade,
                classNum = if (newClassNum.isNotBlank()) newClassNum.trim() else user.classNum,
                num = if (newNum.isNotBlank()) newNum.trim() else user.num
            )
            repository.insertUser(updatedUser)
            currentUser = updatedUser
            currentSchool = repository.getSchoolById(updatedUser.schoolId)
            showToast("✅ 학년/반/번호 및 회원 정보가 성공적으로 수정되었습니다.")
            onSuccess()
        }
    }
    ...
    fun sendChatMessage(text: String, fileName: String? = null, fileUri: String? = null, isImage: Boolean = false) {
        ...
        // 1. Add sender message with delivery status
        ...
        showToast("✉️ [$recipientLabel] 회원에게 메시지를 전송했습니다.")
    }
    ...
    // FRIENDS MANAGEMENT
    // -------------------------------------------------------------
    fun addFriend(friendUid: String, friendName: String, friendPhone: String = "") {
        val me = currentUser ?: return
        if (friendUid == me.uid) {
            showToast("⚠️ 자기 자신은 친구로 추가할 수 없습니다.")
            return
        }
        viewModelScope.launch {
            repository.addFriend(me.uid, friendUid, friendName, friendPhone)
            showToast("👥 '$friendName' 님을 친구 목록에 추가했습니다.")
        }
    }
    ...
    fun removeFriend(friendUid: String, friendName: String = "") {
        val me = currentUser ?: return
        viewModelScope.launch {
            repository.removeFriend(me.uid, friendUid)
            showToast("🗑️ '$friendName' 님을 친구 목록에서 삭제했습니다.")
        }
    }

    fun isFriend(friendUid: String): Boolean {
        return friendsList.value.any { it.friendUid == friendUid }
    }

    // -------------------------------------------------------------
    // CALL & AUDIO CONTROLS (FIREBASE REALTIME DATABASE SIGNALING)
    // -------------------------------------------------------------
    private fun getFirebaseDb(): com.google.firebase.database.FirebaseDatabase? {
        return try {
            com.google.firebase.database.FirebaseDatabase.getInstance("https://integrated-portal-ea306-default-rtdb.firebaseio.com/")
        } catch (e: Exception) {
            null
        }
    }

    private var incomingCallsListener: com.google.firebase.database.ChildEventListener? = null

    private fun setupGlobalCallSignaling() {
        val db = getFirebaseDb() ?: return
        val callsRef = db.getReference("calls")

        incomingCallsListener = object : com.google.firebase.database.ChildEventListener {
            override fun onChildAdded(snapshot: com.google.firebase.database.DataSnapshot, previousChildName: String?) {
                handleCallSignalSnapshot(snapshot)
            }
    ...
    private fun handleCallSignalSnapshot(snapshot: com.google.firebase.database.DataSnapshot) {
        val callId = snapshot.child("callId").value?.toString() ?: snapshot.key ?: return
        val callerUid = snapshot.child("callerUid").value?.toString() ?: ""
        val callerName = snapshot.child("callerName").value?.toString() ?: "상대방"
        val targetUid = snapshot.child("targetUid").value?.toString() ?: ""
        val targetName = snapshot.child("targetName").value?.toString() ?: ""
        val type = snapshot.child("type").value?.toString() ?: "audio"
        val status = snapshot.child("status").value?.toString() ?: ""

        val myUid = currentUser?.uid ?: ""
        val myName = currentUser?.displayName ?: ""

        // 1. 내가 수신자(Target)인 경우
        val isTargetMe = (myUid.isNotEmpty() && targetUid == myUid) ||
                         (myName.isNotEmpty() && targetName.contains(myName, ignoreCase = true))

        if (isTargetMe && callerUid != myUid) {
            if (status == "RINGING" && callState == "IDLE") {
                activeCallRoomId = callId
                activeCallType = type
                callingPeerName = callerName
                isCallMinimized = false
                activeCallParticipants.clear()
                activeCallParticipants.add(callerName)
                callState = "RINGING_INCOMING"
                showToast("📲 [전화 수신 알림] '$callerName' 님에게 전화가 들어왔습니다!")
            } else if ((status == "ENDED" || status == "REJECTED") && activeCallRoomId == callId) {
                callState = "IDLE"
                isCallConnected = false
                activeCallRoomId = null
                activeCallParticipants.clear()
                showToast("📴 통화가 종료/거절되었습니다.")
            }
        }

        // 2. 내가 발신자(Caller)인 경우: 상대방 수락 / 거절 / 종료 감지
        if (callerUid == myUid && activeCallRoomId == callId) {
            when (status) {
                "ACCEPTED" -> {
                    if (callState != "CONNECTED") {
                        callState = "CONNECTED"
                        isCallConnected = true
                        showToast("📞 '$targetName' 님이 전화를 받았습니다! (통화 연결됨)")
                    }
                }
                "REJECTED" -> {
                    callState = "IDLE"
                    ...
                    activeCallRoomId = null
                    activeCallParticipants.clear()
                    showToast("📴 '$targetName' 님이 통화를 거절하였습니다.")
                }
                "ENDED" -> {
                    callState = "IDLE"
                    isCallConnected = false
    ...
    var webRtcManager by mutableStateOf<com.example.webrtc.WebRtcManager?>(null)

    fun initWebRtcIfNeeded(context: Context) {
        if (webRtcManager == null) {
            webRtcManager = com.example.webrtc.WebRtcManager(context.applicationContext)
        }
    }

    fun startWebRtcConnection(isCaller: Boolean, localRenderer: org.webrtc.SurfaceViewRenderer? = null, remoteRenderer: org.webrtc.SurfaceViewRenderer? = null) {
        val roomId = activeCallRoomId ?: return
        val db = getFirebaseDb() ?: return
        val manager = webRtcManager ?: return

        manager.startLocalAudioAndVideo(isVideo = activeCallType == "video", localRenderer = localRenderer)
        manager.initPeerConnection(callId = roomId, isCaller = isCaller, db = db, remoteRenderer = remoteRenderer)
    }

    fun startCallWithPeer(peerName: String, targetUid: String? = null, type: String = "audio") {
        val roomId = "room_" + UUID.randomUUID().toString().substring(0, 8)
        activeCallRoomId = roomId
        activeCallType = type
        callingPeerName = peerName
        isCallMinimized = false
        activeCallParticipants.clear()
        activeCallParticipants.add(peerName)
        callState = "RINGING_OUTGOING"
        isCallConnected = false
        showToast("📞 '$peerName' 님에게 전화를 거는 중... (상대방 수신 응답 대기)")

        val db = getFirebaseDb()
        if (db != null) {
            val callData = mapOf(
                "callId" to roomId,
                "callerUid" to (currentUser?.uid ?: "anon"),
                "callerName" to (currentUser?.displayName ?: "발신자"),
                "targetUid" to (targetUid ?: ""),
                "targetName" to peerName,
                "type" to type,
                "status" to "RINGING",
                "timestamp" to System.currentTimeMillis()
            )
            db.getReference("calls").child(roomId).setValue(callData)
        }
    }
    ...
    fun startCall(type: String) {
        ...
        activeCallRoomId = roomId
        activeCallType = type
        callingPeerName = userNames.joinToString(", ")
        isCallMinimized = false
        activeCallParticipants.clear()
        activeCallParticipants.addAll(userNames)
        callState = "RINGING_OUTGOING"
        isCallConnected = false
        showToast("📞 [$callingPeerName] 님에게 전화를 거는 중... (수신 응답 대기)")
    ...
    fun acceptCall() {
        ...
        if (!roomId.isNullOrEmpty()) {
            getFirebaseDb()?.getReference("calls")?.child(roomId)?.child("status")?.setValue("ACCEPTED")
        }
        showToast("📞 통화를 수락했습니다! (상대방과 연결 완료)")
    }

    fun declineCall() {
        val roomId = activeCallRoomId
        if (!roomId.isNullOrEmpty()) {
            getFirebaseDb()?.getReference("calls")?.child(roomId)?.child("status")?.setValue("REJECTED")
        }
        webRtcManager?.close()
        webRtcManager = null
        callState = "IDLE"
        ...
        activeCallType = null
        activeCallParticipants.clear()
        showToast("📴 통화를 거절하였습니다.")
    }

    fun removeCallParticipant(name: String) {
        activeCallParticipants.remove(name)
        showToast("🗑️ '$name' 님이 통화에서 내보내졌습니다 / 삭제되었습니다.")
        if (activeCallParticipants.isEmpty()) {
            hangupCall()
        }
    }
    ...
    fun hangupCall() {
        val roomId = activeCallRoomId
        if (!roomId.isNullOrEmpty()) {
            getFirebaseDb()?.getReference("calls")?.child(roomId)?.child("status")?.setValue("ENDED")
        }
        webRtcManager?.close()
        webRtcManager = null
        callState = "IDLE"
    ...
    fun toggleSpeakerphone(context: Context) {
        ...
        showToast(if (isSpeakerphoneOn) "🔊 외장 스피커폰으로 전환되었습니다." else "👂 수신기(일반 전화용 스피커)로 전환되었습니다.")
    }

    fun simulateIncomingCall(callerName: String = "김철수 학생") {
        callingPeerName = callerName
        activeCallType = "audio"
        activeCallRoomId = "room_inc_" + UUID.randomUUID().toString().substring(0, 4)
        activeCallParticipants.clear()
        activeCallParticipants.add(callerName)
        callState = "RINGING_INCOMING"
        showToast("📲 [전화 수신 알림] '$callerName' 님에게 전화가 들어왔습니다.")
    ...
    // -------------------------------------------------------------
    // SCHOOL MAP SEARCH NAME CLEANER
    // -------------------------------------------------------------
    fun cleanSchoolNameForMap(rawName: String): String {
        if (rawName.isBlank()) return "학교"
        var cleaned = rawName
            .replace(Regex("\\{[^}]*\\}"), "")
            .replace(Regex("\\[[^\\]]*\\]"), "")
            .replace(Regex("\\([^)]*\\)"), "")
            .trim()
        cleaned = cleaned.replace(Regex(".*?교육청\\s*"), "").trim()
        if (cleaned.isBlank()) {
            cleaned = rawName.replace(Regex("[{}\\[\\]()]"), "").split("/").lastOrNull()?.trim() ?: rawName
        }
        return cleaned
    }

    // -------------------------------------------------------------
    // SCHOOL MEAL / LUNCH CHECK METHODS WITH DATE PICKER
    // -------------------------------------------------------------
    var schoolMealMenu by mutableStateOf<List<String>>(emptyList())
    var isMealLoading by mutableStateOf(false)
    var schoolMealInfoMessage by mutableStateOf("")

    fun setMealDateAndFetch(daysOffset: Int = 0, specificYmd: String? = null) {
        val sdfYmd = SimpleDateFormat("yyyyMMdd", Locale.getDefault())
        val sdfDisp = SimpleDateFormat("yyyy년 MM월 dd일 (E)", Locale.KOREAN)
        val cal = Calendar.getInstance()

        if (!specificYmd.isNullOrEmpty()) {
            try {
                val parsed = sdfYmd.parse(specificYmd)
            ...
            try {
                val current = sdfYmd.parse(selectedMealDateYmd)
        ...
        selectedMealDateYmd = sdfYmd.format(cal.time)
        selectedMealDateFormatted = sdfDisp.format(cal.time)
        fetchSchoolMeal(selectedMealDateYmd)
    }

    fun fetchSchoolMeal(dateYmd: String = selectedMealDateYmd) {
        val school = currentSchool ?: return
        viewModelScope.launch {
            isMealLoading = true
            try {
                val cleanCode = school.inviteCode.removePrefix("SEL").removePrefix("ACA")
                val apiKey = activeSchoolApiKey
                val url = "https://open.neis.go.kr/hub/mealServiceDietInfo?KEY=$apiKey&Type=json&pIndex=1&pSize=5&SD_SCHUL_CODE=$cleanCode&MLSV_YMD=$dateYmd"
                val response = fetchFromNeis(url)
                val json = JSONObject(response)
                if (json.has("mealServiceDietInfo")) {
                    val array = json.getJSONArray("mealServiceDietInfo").getJSONObject(1).getJSONArray("row")
                    val meals = mutableListOf<String>()
                    for (i in 0 until array.length()) {
                        val row = array.getJSONObject(i)
                        val dish = row.optString("DDISH_NM", "").replace("<br/>", "\n").replace(Regex("\\([0-9.]+\\)"), "")
                        val mealType = row.optString("MMEAL_SC_NM", "중식")
                        meals.add("🍱 [$mealType]\n$dish")
                    }
                    schoolMealMenu = meals
                    schoolMealInfoMessage = "✅ 교육청 NEIS 실시간 급식 정보를 불러왔습니다."
                } else {
                    schoolMealMenu = listOf(
                        "🍱 [중식]\n🍚 혼합잡곡밥\n🥣 얼큰쇠고기무국\n🍗 수제치킨까스 & 머스타드\n🥗 매콤콩나물무침\n🥬 포기김치\n🥛 친환경 100% 우유 / 🧃 청포도 에이드"
                    )
                    schoolMealInfoMessage = "🏫 급식 정보가 없거나 휴업일입니다."
                }
            } catch (e: Exception) {
                schoolMealMenu = listOf(
                    "🍱 [중식]\n🍚 현미찹쌀밥\n🥣 차돌된장찌개\n🥩 뚝배기불고기\n🥗 깍두기 & 상추쌈\n🥛 우유 / 🍊 제철과일"
                )
                schoolMealInfoMessage = "🏫 학교 급식 정보입니다."
            } finally {
                isMealLoading = false
            }
        }
    ...
    // TIMETABLE METHODS
    // -------------------------------------------------------------
    val timetableList: StateFlow<List<TimetableEntity>> = snapshotFlow {
        Triple(currentSchool?.schoolId ?: "", currentUser?.grade ?: "1", currentUser?.classNum ?: "1")
    }.flatMapLatest { (schId, gr, cl) ->
        if (schId.isNotEmpty()) {
            repository.getTimetableFlow(schId, gr, cl)
        } else {
            flowOf(emptyList())
        }
    }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())

    fun saveTimetableEntry(dayOfWeek: String, period: Int, subject: String, teacherName: String = "", classroom: String = "") {
        val user = currentUser ?: return
        viewModelScope.launch {
            val id = "${user.schoolId}-${user.grade}-${user.classNum}-$dayOfWeek-$period"
            ...
            showToast("📅 ${dayOfWeek}요일 ${period}교시 [$subject] 시간표가 저장되었습니다.")
        }
    }

    fun seedDefaultTimetableForClass() {
        val user = currentUser ?: return
        viewModelScope.launch {
            val days = listOf("월", "화", "수", "목", "금")
            val sampleSubjects = listOf(
                listOf("국어", "수학", "영어", "사회", "과학", "체육", "자율"),
                listOf("수학", "영어", "국어", "역사", "음악", "미술", "동아리"),
                listOf("영어", "국어", "수학", "과학", "기술가정", "체육", "진로"),
                listOf("사회", "과학", "국어", "수학", "영어", "한문", "자율"),
                listOf("과학", "사회", "체육", "국어", "수학", "영어", "창체")
            )
            val list = mutableListOf<TimetableEntity>()
            days.forEachIndexed { dayIdx, day ->
                val subjects = sampleSubjects[dayIdx]
                subjects.forEachIndexed { pIdx, sub ->
                    val period = pIdx + 1
                    val id = "${user.schoolId}-${user.grade}-${user.classNum}-$day-$period"
                    list.add(
                        TimetableEntity(
                            id = id,
                            schoolId = user.schoolId,
                            ...
                            classroom = "${user.grade}-${user.classNum} 교실"
                        )
                    )
                }
            }
            repository.insertTimetables(list)
            showToast("📅 ${user.grade}학년 ${user.classNum}반 표준 시간표가 생성되었습니다.")
        }
    }
